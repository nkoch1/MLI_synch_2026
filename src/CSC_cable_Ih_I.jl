"""Module for modeling cerebellar stellate cells (CSC) with dendritic cable and HCN channels.

Implements ODE-based compartmental models with passive dendritic cable equation and
active somatic/dendritic conductances. Supports various input types (current injection, chirps, EPSCs).
Based on Angelo et al. (2007) for Ih kinetics and Goldberg et al. (2007) for cable formulation.
"""
module CSC_cable_Ih_I
...
const D = 5.3e-9 #0.53 *1e-4*1e-4 # μm^2/ms-> cm^2/ms
...
################################################################################
# Chirp Input Functions
# Generates frequency-swept sinusoidal current for frequency response analysis
# Typical parameters:
#   phi0: initial phase (rad)
#   f0: starting frequency (kHz)
#   f1: ending frequency (kHz)
#   T: total sweep time (ms)
#   c = (f1 - f0) / T: chirp rate (kHz/ms)
#   Iscale: current amplitude (nA)
################################################################################

"""Chirp current waveform: I(t) = Iscale * sin(phi0 + 2π(ct²/2 + f0*t))
   Scalar version for single timepoint."""
function chirp(t::Float64, Iscale, phi0, c, f0)
    return Iscale * sin(phi0 + 2 * π * ((c / 2) * t^2 + f0 * t))
end

"""Chirp current waveform - vector version."""
function chirp(t::Vector{Float64}, Iscale, phi0, c, f0)
    return Iscale .* sin.(phi0 .+ 2 .* π .* ((c ./ 2) .* t.^2 .+ f0 .* t))
end

"""Chirp current waveform - array version."""
function chirp(t::Array{Float64}, Iscale, phi0, c, f0)
    return Iscale .* sin.(phi0 .+ 2 .* π .* ((c ./ 2) .* t.^2 .+ f0 .* t))
end



"""Time derivative of chirp waveform: dI/dt = 2π*Iscale*(ct + f0)*cos(...)
   Used to constrain soma voltage derivative in CSCcable_soma! models.
   Scalar version."""
function chirp_dV(t::Float64, μ, Iscale, phi0, c, f0)
    return 2 * π * Iscale * (c * t + f0) * cos(phi0 + 2 * π * t * ((c / 2) * t + f0))
end

"""Time derivative of chirp waveform - vector version."""
function chirp_dV(t::Vector{Float64}, μ, Iscale, phi0, c, f0)
    return 2 .* π .* Iscale .* (c .* t .+ f0) .* cos.(phi0 .+ 2 .* π .* t .* ((c ./ 2) .* t .+ f0))
end

"""Time derivative of chirp waveform - array version."""
function chirp_dV(t::Array{Float64}, μ, Iscale, phi0, c, f0)
    return 2 .* π .* Iscale .* (c .* t .+ f0) .* cos.(phi0 .+ 2 .* π .* t .* ((c ./ 2) .* t .+ f0))
end
...
Current is injected at location input_loc or as zero (if I_type ≠ "chirp").
"""
function CSCcable!(dr, r, p, t)
    gNa::Float64, gK::Float64, gleak::Float64, gA::Float64, gT::Float64, gHVA::Float64, gSK::Float64, gh::Float64, gKd::Float64, gleakd::Float64, gAd::Float64, gTd::Float64, gHVAd::Float64, gSKd::Float64,ghd::Float64, soma_r::Float64, dend_r::Float64, I::Float64, L::Float64, Eleak::Float64, Eleakd::Float64, numseg, Eh::Float64, Vm::Float64, Vh::Float64, Vn::Float64, VnA::Float64, VhA::Float64, VmT::Float64, VhT::Float64, VmHVA::Float64, km::Float64,kmT::Float64, kh::Float64, kn::Float64, knA::Float64, khA::Float64,khT::Float64, kmHVA::Float64,  EK::Float64, ENa::Float64, ECa::Float64, I_type, input_loc = p
    ...
    I_input = zeros(numseg+1)
    if I_type == "chirp"
        I_input[input_loc] = chirp(t, Iscale, phi0, c, f0)  # applied chirp current
        u[2, end] = I_input[input_loc]  # save for diagnostics
    end

    # Pre-compute voltage-dependent steady-state variables
    # These are fast (instantaneous) gating variables
    mSS = @inbounds @views 1 / (1 + exp((u[1,1] - Vm) / -km))  # Na activation (steady-state)
    ...
    @inbounds @views begin
        ...
        du[1,1] = (I
                    - gK * (u[3,1]^4) * (u[1,1] - EK)              # K current (n^4)
                    - gNa * (mSS^3) * u[2,1] * (u[1,1] - ENa)      # Na current (m^3*h)
                    - gleak * (u[1,1] - Eleak)                     # leak current
                    - gA * u[4,1] * u[5,1] * (u[1,1] - EK)         # A-current (m*h)
                    - gT * mTSS[1] * u[6,1] * (u[1,1] - ECa)       # T-type Ca (m*h)
                    - gSK * (u[8,1]^5 / ((0.45^5) + (u[8,1]^5))) * (u[1,1] - EK)  # SK Ca-dependent K
                    - gHVA * u[7,1] * (u[1,1] - ECa)               # HVA Ca (m)
                    - u[9,1] * gh * (u[1,1] - Eh)                  # Ih current (h)
                    + κ * (u[1,2] - u[1,1]) / Δx                   # axial current from dendrite
                    + I_input[1]
                    ) / capm;
        ...
        for ii in 1:Int(numseg+1)::Int64
            # Na inactivation (h_Na): voltage-dependent time constant
            du[2, ii] = ((1 / (1 + exp((u[1,ii] - Vh) / kh))) - u[2,ii]) / 
                        (0.1 + (2 * 322 * 46) / (4 * pi * (u[1,ii] + 74)^2 + 46^2))
            # K activation (n_K): voltage-dependent time constant
            du[3, ii] = ((1 / (1 + exp(-(u[1,ii] - Vn) / kn))) - u[3,ii]) / 
                        (6 / (1 + exp((u[1,ii] + 23) / 15)))
            # A-current activation (m_A): fixed time constant
            du[4, ii] = ((1 / (1 + exp((u[1,ii] - VnA) / -knA))) - u[4,ii]) / 5
            # A-current inactivation (h_A): fixed time constant
            du[5, ii] = ((1 / (1 + exp((u[1,ii] - VhA) / khA))) - u[5,ii]) / 10
            # T-type Ca inactivation (h_T): fixed time constant
            du[6, ii] = ((1 / (1 + exp((u[1,ii] - VhT) / khT))) - u[6,ii]) / 15
            # HVA Ca activation (m_HVA): fixed time constant
            du[7, ii] = ((1 / (1 + exp(-(u[1,ii] - VmHVA) / kmHVA))) - u[7,ii]) / 4
            # HCN (Ih) inactivation (h_Ih): voltage-dependent time constant
            du[9, ii] = (hinf(u[1,ii]) - u[9,ii]) / tauh(u[1,ii])
        end
        ...
        du[1,2] = ((λ^2)/(Δx^2) * (u[1,3]- 2*u[1,2] + u[1,1])
            - gleakd * γs * (u[1,2] - Eleakd)
            - gKd * γs * (u[3,2]^4) * (u[1,2]  - EK)
            - gAd * γs * u[4,2] * u[5,2] * (u[1,2]  - EK)
            - gTd * γs * mTSS[2] * u[6,2] * (u[1,2]-ECa)
            - gSKd * γs * (u[8,2]^5/((0.45^5)+(u[8,2]^5))) * (u[1,2]  - EK)
            - gHVAd * γs * u[7,2] * (u[1,2] - ECa)
            - u[9,2] * ghd * γs * (u[1,2] - Eh)
            + I_input[2]
            ) / τm
        ...
        for ii in 3:Int(numseg)::Int64
            du[1,ii] = ((λ^2) / (Δx^2) * (u[1,ii+1] - 2*u[1,ii] + u[1,ii-1])
                        - gleakd * γs * (u[1,ii] - Eleakd)         # leak current (dendritic)
                        - gKd * γs * (u[3,ii]^4) * (u[1,ii] - EK)  # K current (dendritic)
                        - gAd * γs * u[4,ii] * u[5,ii] * (u[1,ii] - EK)  # A-current (dendritic)
                        - gTd * γs * mTSS[ii] * u[6,ii] * (u[1,ii] - ECa)  # T-type Ca (dendritic)
                        - gSKd * γs * (u[8,ii]^5 / ((0.45^5) + (u[8,ii]^5))) * (u[1,ii] - EK)  # SK K (dendritic)
                        - gHVAd * γs * u[7,ii] * (u[1,ii] - ECa)   # HVA Ca (dendritic)
                        - u[9,ii] * ghd * γs * (u[1,ii] - Eh)      # Ih current (dendritic)
                        + I_input[ii]
                        ) / τm
        end
        ...
        du[1,end] = ((λ^2) / (Δx^2) * (-u[1,end] + u[1,end-1])
                        - gleakd * γs * (u[1,end] - Eleakd)
                        - gKd * γs * (u[3,end]^4) * (u[1,end] - EK)
                        - gAd * γs * u[4,end] * u[5,end] * (u[1,end] - EK)
                        - gTd * γs * mTSS[end] * u[6,end] * (u[1,end] - ECa)
                        - gSKd * γs * (u[8,end]^5 / ((0.45^5) + (u[8,end]^5))) * (u[1,end] - EK)
                        - gHVAd * γs * u[7,end] * (u[1,end] - ECa)
                        - u[9,end] * ghd * γs * (u[1,end] - Eh)
                        + I_input[end]
                        ) / τm
    end
end
...
"""Soma-constrained version: soma voltage derivative forced to follow chirp waveform.
   Dendrite evolves with passive cable and active conductances.
   Useful for characterizing dendritic filtering in response to prescribed soma voltage.
"""
function CSCcable_soma!(dr, r, p, t)
    gNa::Float64, gK::Float64, gleak::Float64, gA::Float64, gT::Float64, gHVA::Float64, gSK::Float64, gh::Float64, gKd::Float64, gleakd::Float64, gAd::Float64, gTd::Float64, gHVAd::Float64, gSKd::Float64,ghd::Float64, soma_r::Float64, dend_r::Float64, I::Float64, L::Float64, Eleak::Float64, Eleakd::Float64, numseg::Float64, Eh::Float64, Vm::Float64, Vh::Float64, Vn::Float64, VnA::Float64, VhA::Float64, VmT::Float64, VhT::Float64, VmHVA::Float64, km::Float64,kmT, kh::Float64, kn::Float64, knA::Float64, khA::Float64,khT::Float64, kmHVA::Float64,  EK::Float64, ENa::Float64, ECa::Float64, I_type, input_loc, μ = p
    r = check_gating_bounds(r, numseg)
    u = @view r[:, :]
    du = @view dr[:, :]

    # Geometric parameters: convert to cm, compute cable properties
    Δx = L / numseg  # compartment length (cm)
    a = dend_r / 10000  # dendritic radius (μm -> cm)
    As = soma_r / 10000  # somatic radius (μm -> cm)
    rmem = 1 / (gleakd / 1000)  # membrane resistance (Ω·cm²)
    τm = rmem * capm / 1000  # membrane time constant (ms)
    λ = sqrt(a * rmem / (2 * rl))  # space constant (cm)
    κ = 1.0 / (rl / 1000) * (a / (2 * As))^2  # soma-dendrite coupling coefficient
    γs = 1 / (gleakd)  # conductance scaling factor

    # FORCE soma voltage to follow chirp derivative
    # This eliminates sodium-generated voltage dynamics, isolating dendritic filtering
    du[1, 1] = chirp_dV(t, μ, Iscale, phi0, c, f0)
    mTSS = @inbounds @views @. 1 / (1 + exp(-(u[1, :] - VmT) / kmT));
    ...
    for ii in 3:Int(numseg)::Int64
        du[1,ii] = ((λ^2) / (Δx^2) * (u[1,ii+1] - 2 *u[1,ii] + u[1,ii-1])
        - gleakd * γs * (u[1,ii] - Eleakd)
        - gKd * γs * (u[3,ii]^4) * (u[1,ii]  - EK)
        - gAd * γs * (u[4,ii]) * (u[5,ii]) * (u[1,ii]  - EK)
        - gTd * γs * (mTSS[ii]) * (u[6,ii]) * (u[1,ii] - ECa)
        - gSKd * γs * (u[8,ii]^5 / ((0.45^5) + (u[8,ii]^5))) * (u[1,ii]  - EK)
        - gHVAd * γs * (u[7,ii]) * (u[1,ii] - ECa)
        - u[9,ii] * ghd * γs * (u[1,ii] - Eh)
        ) / τm
    end
...
function CSCcable_soma_alt!(dr, r, p, t)
    gNa::Float64, gK::Float64, gleak::Float64, gA::Float64, gT::Float64, gHVA::Float64, gSK::Float64, gh::Float64, gKd::Float64, gleakd::Float64, gAd::Float64, gTd::Float64, gHVAd::Float64, gSKd::Float64,ghd::Float64, soma_r::Float64, dend_r::Float64, I::Float64, L::Float64, Eleak::Float64, Eleakd::Float64, numseg::Float64, Eh::Float64, Vm::Float64, Vh::Float64, Vn::Float64, VnA::Float64, VhA::Float64, VmT::Float64, VhT::Float64, VmHVA::Float64, km::Float64,kmT::Float64, kh::Float64, kn::Float64, knA::Float64, khA::Float64,khT::Float64, kmHVA::Float64,  EK::Float64, ENa::Float64, ECa::Float64, I_type, input_loc, μ = p
    r = check_gating_bounds(r, numseg)
    u = @view r[:, :]
    du = @view dr[:, :]
    ...
    du[1, 1] = chirp_dV(t,μ, Iscale, phi0, c, f0) #I_input[input_loc] # save chirp to u[2, end]
    mTSS = @inbounds @views @. 1 / (1 + exp(-(u[1, :] - VmT) / kmT));
    ...
    for ii in 1:Int(numseg+1)::Int64
        ...
        du[6, ii] = ((1 / (1 + exp((u[1, ii] - VhT) / khT))) - u[6, ii]) / 15; #hT
        du[7, ii] = ((1 / (1 + exp(-(u[1, ii] - VmHVA) / kmHVA))) - u[7,ii]) / 4; #mHV
        du[9, ii] = (hinf(u[1, ii]) - u[9, ii]) / tauh_alt(u[1, ii]) #hcn
    end
...
function CSCcable_soma_alt2!(dr, r, p, t)
    gNa::Float64, gK::Float64, gleak::Float64, gA::Float64, gT::Float64, gHVA::Float64, gSK::Float64, gh::Float64, gKd::Float64, gleakd::Float64, gAd::Float64, gTd::Float64, gHVAd::Float64, gSKd::Float64,ghd::Float64, soma_r::Float64, dend_r::Float64, I::Float64, L::Float64, Eleak::Float64, Eleakd::Float64, numseg::Float64, Eh::Float64, Vm::Float64, Vh::Float64, Vn::Float64, VnA::Float64, VhA::Float64, VmT::Float64, VhT::Float64, VmHVA::Float64, km::Float64,kmT::Float64, kh::Float64, kn::Float64, knA::Float64, khA::Float64,khT::Float64, kmHVA::Float64,  EK::Float64, ENa::Float64, ECa::Float64, I_type, input_loc, μ = p
    r = check_gating_bounds(r, numseg)
    u = @view r[:, :]
    du = @view dr[:, :]
    ...
    for ii in 1:Int(numseg+1)::Int64
        ...
        du[6, ii] = ((1 / (1 + exp((u[1, ii] - VhT) / khT))) - u[6, ii]) / 15; #hT
        du[7, ii] = ((1 / (1 + exp(-(u[1, ii] - VmHVA) / kmHVA))) - u[7,ii]) / 4; #mHV
        du[9, ii] = (hinf(u[1, ii]) - u[9, ii]) / tauh_alt2(u[1, ii]) #hcn
    end
...
function CSCcable_EPSP!(dr, r, p, t)
    gNa::Float64, gK::Float64, gleak::Float64, gA::Float64, gT::Float64, gHVA::Float64, gSK::Float64, gh::Float64, gKd::Float64, gleakd::Float64, gAd::Float64, gTd::Float64, gHVAd::Float64, gSKd::Float64,ghd::Float64, soma_r::Float64, dend_r::Float64, I::Float64, L::Float64, Eleak::Float64, Eleakd::Float64, numseg::Float64, Eh::Float64, Vm::Float64, Vh::Float64, Vn::Float64, VnA::Float64, VhA::Float64, VmT::Float64, VhT::Float64, VmHVA::Float64, km::Float64,kmT::Float64, kh::Float64, kn::Float64, knA::Float64, khA::Float64,khT::Float64, kmHVA::Float64,  EK::Float64, ENa::Float64, ECa::Float64,gsyn, tsyn, tce, tce2 = p
    r = check_gating_bounds(r, numseg)
    u = @view r[:, :]
    du = @view dr[:, :]
    ...
    if t < tsyn
        ...
        Ied = -gsyn * γs * (exp( -tce * (t - tsyn)) - exp( - tce2 * (t - tsyn)))
    end
    if isnan(Ied)
        Ied =0.
    end
    ...
    for ii in 3:Int(numseg)::Int64  
            du[1,ii] = (
                (λ^2) / (Δx^2) * (u[1,ii+1] - 2 *u[1,ii] + u[1,ii-1])
            - gleakd * γs * (u[1,ii] - Eleakd)
            - gKd * γs * (u[3,ii]^4) * (u[1,ii]  - EK)
            - gAd * γs * (u[4,ii]) * (u[5,ii]) * (u[1,ii]  - EK)
            - gTd * γs * (mTSS[ii]) * (u[6,ii]) * (u[1,ii] - ECa)
...
"""
function steady_state_init(V0, p, tspan, numseg; alg=QNDF())
    p = tuple(p...)
    u0 = steady_state_init_u0_gating(V0, numseg, p[24:end])
    ss_cb = TerminateSteadyState(min_t=100)
    prob_ic = ODEProblem(CSCcable!, u0, tspan, p, callback=ss_cb, save_everystep=false, save_end=true, save_start=false)
    sol_ic = solve(prob_ic, alg, dtmin=1e-60, dt=1e-20, abstol=1e-12, reltol=1e-10, maxiters=10000000, alg_hints=[:stiff])
    return sol_ic.u[end]
    nothing
end
    

"""Steady-state initialization for EPSC/synaptic input protocols.
"""
function steady_state_init_EPSP(V0, p, tspan, numseg; abstol=1e-12, reltol=1e-10, alg=QNDF())
    p = tuple(p...)
    u0 = steady_state_init_u0_gating(V0, numseg, p[24:end])
    ss_cb = TerminateSteadyState(min_t=100)
    prob_ic = ODEProblem(CSCcable_EPSP!, u0, tspan, p, callback=ss_cb, save_everystep=false, save_end=true, save_start=false)
    sol_ic = solve(prob_ic, alg, dtmin=1e-100, dt=1e-20, abstol=abstol, reltol=reltol, maxiters=10000000, alg_hints=[:stiff])
    return sol_ic.u[end]
    nothing
end
...
function step_I(step_start, step_end, I_mag; saveall=false)
    if saveall
        ...
        # Turn on saving when current starts
...
"""
function fIcurve(pro, Iarray, par, step_start, step_end; somaidx=1,
    ...
    function prob_func(pro, i, repeat)
        ...
        remake(proo, p=par_new, callback=step_I(step_start, step_end, Iarray[i]))
    end
    ...
    for i in eachindex(sim)
        if SciMLBase.successful_retcode(sim[i].retcode)
            ...
            if length(peaks) >= 2  # need ≥ 2 spikes to estimate frequency
                ...
                ISI = diff(spiket)  # interspike intervals (ms)
                Finst = 1000 ./ ISI  # convert to frequency (Hz)
                F[i] = mean(Finst)
            else
                F[i] = 0.  # no spiking
            end
