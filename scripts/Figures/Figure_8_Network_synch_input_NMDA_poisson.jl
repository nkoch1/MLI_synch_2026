...
PLV_array = wload(datadir("simulations", "Network", "new",  savename(fname_save, param_save, "jld2")), "PLV_after_50_array");
PLV_array_mean = wload(datadir("simulations", "Network", "new",  savename(fname_save, param_save, "jld2")), "PLV_mean_after_50_array");
gsyn_array = wload(datadir("simulations", "Network", "new",  savename(fname_save, param_save, "jld2")), "gsyn_array");
ggap_array = wload(datadir("simulations", "Network", "new",  savename(fname_save, param_save, "jld2")), "ggap_array");


delta_PLV_array = PLV_array .- PLV_before_array
delta_PLV_array_mean = PLV_array_mean .- PLV_before_array_mean
...
cmap = matplotlib.colors.LinearSegmentedColormap.from_list("", ["#cece8e", "#595959"])
N = length(gsyn_array)
...
gs_all  = fig.add_gridspec(1, 2,left=0.075, right=0.975, wspace=0.35, hspace=0.5,  width_ratios=[0.45, 0.55],top=0.95, bottom=0.1)
...
ax_ggap_sum = fig.add_subplot(py"$(gs_right)[0, 0]")
ax_ggap_sum = pyplot_fxns.remove_axis_box(ax_ggap_sum; s=["top", "right"])

ax_gGABA_sum = fig.add_subplot(py"$(gs_right)[2, 0]")
...
ax_ggap_sum_PLV = fig.add_subplot(py"$(gs_right)[0, 1]")
ax_ggap_sum_PLV = pyplot_fxns.remove_axis_box(ax_ggap_sum_PLV; s=["top", "right"])

ax_gGABA_sum_PLV = fig.add_subplot(py"$(gs_right)[2, 1]")
...
ax_ggap_sum.set_xticklabels(ax_ggap_sum.get_xticklabels(), rotation=45, ha="center", va="top") 
...
ax_ggap_gGABA_sum.set_xticklabels(ax_ggap_gGABA_sum.get_xticklabels(), rotation=45, ha="center", va="top") 



# plot summary
sns.boxplot(delta_PLV_array[gsyn_ind_norm,:,:]', ax=ax_ggap_sum_PLV, saturation=0.5, showfliers=false, palette=n_colors_ggap)
sns.stripplot(delta_PLV_array[gsyn_ind_norm,:,:]', ax=ax_ggap_sum_PLV, palette=n_colors_ggap, size=m_size, alpha=0.75)
ax_ggap_sum_PLV.set_xticks(collect(0:length(ggap_array)-1))
ax_ggap_sum_PLV.set_xticklabels(ggap_array_labels)
ax_ggap_sum_PLV.set_xlabel(L"g_{gap}" * " (nS)")
ax_ggap_sum_PLV.set_ylim(PLV_lim)
ax_ggap_sum_PLV.set_ylabel("ΔPLV")
ax_ggap_sum_PLV.set_xticklabels(ax_ggap_sum_PLV.get_xticklabels(), rotation=45, ha="center", va="top") 

sns.boxplot(delta_PLV_array[:,ggap_ind_norm,:]', ax=ax_gGABA_sum_PLV, saturation=0.5, showfliers=false, palette=n_colors_gsyn)
sns.stripplot(delta_PLV_array[:,ggap_ind_norm,:]', ax=ax_gGABA_sum_PLV, palette=n_colors_gsyn, size=m_size, alpha=0.75)
ax_gGABA_sum_PLV.set_xticks(collect(0:length(gsyn_array)-1))
ax_gGABA_sum_PLV.set_xticklabels(gsyn_array_labels)
ax_gGABA_sum_PLV.set_xlabel(L"g_{GABA}" * " (nS)")
ax_gGABA_sum_PLV.set_ylim(PLV_lim)
ax_gGABA_sum_PLV.set_ylabel("ΔPLV")
ax_gGABA_sum_PLV.set_xticklabels(ax_gGABA_sum_PLV.get_xticklabels(), rotation=45, ha="center", va="top") 
...
ax_ggap_gGABA_sum_PLV.set_xticks(collect(0:length(gsyn_array)-1) .+ 0.5)
ax_ggap_gGABA_sum_PLV.set_xticklabels(gsyn_array_labels)
ax_ggap_gGABA_sum_PLV.set_xlabel(L"g_{GABA}" * " (nS)")

ax_ggap_gGABA_sum_PLV.set_yticks(collect(0:length(ggap_array)-1) .+ 0.5)
ax_ggap_gGABA_sum_PLV.set_yticklabels(ggap_array_labels)
ax_ggap_gGABA_sum_PLV.set_ylabel(L"g_{gap}" * " (nS)")
...
ax_ggap_gGABA_sum_PLV.set_xticklabels(ax_ggap_gGABA_sum_PLV.get_xticklabels(), rotation=45, ha="center", va="top") 
...
ax_ggap_sum.text(-0.05, 1.075, "$(letters[3])", transform=ax_ggap_sum.transAxes, size=letter_size, weight="bold")
ax_ggap_sum_PLV.text(-0.05, 1.075, "$(letters[4])", transform=ax_ggap_sum_PLV.transAxes, size=letter_size, weight="bold")
...
ax_gGABA_sum.text(-0.05, 1.075, "$(letters[7])", transform=ax_gGABA_sum.transAxes, size=letter_size, weight="bold")
ax_gGABA_sum_PLV.text(-0.05, 1.075, "$(letters[8])", transform=ax_gGABA_sum_PLV.transAxes, size=letter_size, weight="bold")
...
plt.savefig(plotsdir("Figure_8_Network_synch_input_NMDA_poisson.png"), dpi=600)
plt.savefig(plotsdir("Figure_8_Network_synch_input_NMDA_poisson.pdf"), dpi=600)
plt.savefig(plotsdir("Figure_8_Network_synch_input_NMDA_poisson.eps"), dpi=600)
plt.show()
...
# ggap PLV
x = [collect((ggap_array * ones(1, 100))')...]
y = [collect(delta_PLV_array[gsyn_ind_norm,:,:]')...]

rho, p = sp.stats.spearmanr(x,y)

# GABA PLV
x = [collect((gsyn_array * ones(1, 100))')...]
y = [collect(delta_PLV_array[:,ggap_ind_norm,:]')...]
